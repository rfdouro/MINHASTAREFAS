  ...
  <q-layout view="hHh lpR fFf" class="fullscreen">
    <q-header class="bg-primary text-white" height-hint="98">
      <q-toolbar>
        ...
        <DropDownUser :user="user" :funAltera="abreAltera" :funSair="sair"></DropDownUser>
      </q-toolbar>
      <q-tabs v-model="tab" dense class="text-white" active-color="white" indicator-color="white" align="left">
    ...
    <q-page-container>
      <q-tab-panels v-model="tab" animated class="row q-pa-md" swipeable draggable>
        <q-tab-panel name="lista" v-touch-swipe.mouse.right.left="handleSwipe" draggable>
          <div class="text-h6">Tarefas</div>
          <div class="q-gutter-md">
            <q-list bordered separator>
              <q-item :clickable="false" v-ripple="false" v-for="t in tarefas" v-bind:key="t.key" class="q-pa-lg"
                :class="{
                  'bg-red-1 q-pa-lg': t.dataLimite && new Date(t.dataLimite) < new Date(),
                  'bg-blue-1 q-pa-lg': t.dataLimite && new Date(t.dataLimite) >= new Date(),
                  'bg-green-1 q-pa-lg': t.concluida,
                }">
                <q-item-section>
                  <q-item-label>{{ t.titulo }}</q-item-label>
                  <q-item-label caption>{{ t.descricao }}<br />
                  ...
                  <p side class="lt-sm">
                    <q-btn-group push>
                      <q-btn icon="update" color="green" @click="
                        tarefa = t;
                      diagaltera = true;
                      "></q-btn>
                ...
                <q-item-section side class="gt-xs">
                  <q-btn-group push>
                    <q-btn label="Alterar" icon="update" color="green" @click="
                      tarefa = t;
                    diagaltera = true;
                    "></q-btn>
                    <q-btn label="Excluir" icon="delete" color="red" @click="exclui(t)"></q-btn>
                  </q-btn-group>
                </q-item-section>
              </q-item>
            </q-list>
        ...
        <q-tab-panel name="cadastro" v-touch-swipe.mouse.right.left="handleSwipe" draggable>
          <q-card class="bg-blue-1 q-pa-md">
            <div class="text-h6">Nova Tarefa</div>
            <div class="q-gutter-md">
              <q-input filled class="q-mb-lg" type="text" v-model="tarefa.titulo" label="Título *"
                hint="Título da tarefa" lazy-rules
                :rules="[(val) => (val && val.length > 0) || 'Digite um título']"></q-input>
              <q-input filled class="q-mb-lg" type="text" v-model="tarefa.descricao" label="Descrição *"
                hint="Descrição da tarefa" lazy-rules
                :rules="[(val) => (val && val.length > 0) || 'Digite uma descrição']"></q-input>
              <q-input filled class="q-mb-lg" type="date" v-model="tarefa.dataLimite" label="Data limite *"
                hint="Data limite"></q-input>
              <div>
      ...
      <q-dialog v-model="diagaltera" persistent>
        <q-card style="width: 700px; max-width: 80vw" class="bg-green-3">
          ...
          <q-card-section class="q-pt-none">
            <div class="q-gutter-md">
              <q-input filled type="text" v-model="tarefa.titulo" label="Título *" hint="Título da tarefa" lazy-rules
                :rules="[(val) => (val && val.length > 0) || 'Digite um título']"></q-input>
              <q-input color="white" filled type="text" v-model="tarefa.descricao" label="Descrição *"
                hint="Descrição da tarefa" lazy-rules
                :rules="[(val) => (val && val.length > 0) || 'Digite uma descrição']"></q-input>
              <q-input filled type="date" v-model="tarefa.dataLimite" label="Data limite *"
                hint="Data limite"></q-input>
            </div>
          </q-card-section>
          ...
          <q-card-actions align="right" class="bg-green-2">
            ...
            <q-btn flat label="Fechar" v-close-popup @click="
              () => {
                limpa();
                todas();
              }
